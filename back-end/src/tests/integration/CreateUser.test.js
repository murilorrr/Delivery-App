...
describe('Create User - Sua aplicação deve ter o endpoint POST `/user`', () => {
  ...
  it('Será validado que é possível criar um usuário com sucesso', (done) => {
    chai.request(server)
      .post('/user')
      .send(user)
      .end((err, res) => {
  ...
  it('Será validado que não é possível criar um usuário sem o campo email', (done) => {
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.be.deep.equal({ message: "child \"email\" fails because [\"email\" is required]" });
        done();
      });
  });
  ...
  it('Será validado que não é possível criar um usuário sem o campo password', (done) => {
    chai.request(server)
      ...
      .end((err, res) => {
        if (err) done(err);
        expect(res.status).to.be.equal(400);
        expect(res.body).to.be.deep.equal({ message: "child \"password\" fails because [\"password\" is required]" });
        done();
      });
  });
  ...
  it('Será validado que não é possível criar um usuário sem o campo name', (done) => {
    chai.request(server)
      ...
      .end((err, res) => {
        if (err) done(err);
        expect(res.status).to.be.equal(400);
        expect(res.body).to.be.deep.equal({ message: "child \"name\" fails because [\"name\" is required]" });
        done();
      });
  });
  ...
  it('Será validado que não é possível criar um usuário que já existe', (done) => {
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.be.deep.equal({ message: 'User already registered' });
        done();
      });
  })
});
